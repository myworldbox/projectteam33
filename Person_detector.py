...
BR_inside = (int(IM_WIDTH* 2/6), int(IM_HEIGHT * 7/8))

# Define outside box coordinates (top left and bottom right)
TL_outside = (int(IM_WIDTH * 4/6), int(IM_HEIGHT* 1/8))
...
def person_detector(frame):
    ...
    cv2.putText(frame, 'Pause counter: ' + str(pause_counter), (10, 130), font, 0.5, (255, 255, 0), 1, cv2.LINE_AA)
...
if camera_type == 'picamera':
    ...
    for frame1 in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        # Draw FPS
        cv2.putText(frame, "FPS: {0:.2f}".format(frame_rate_calc), (10, 50), font, 1, (255, 255, 0), 1, cv2.LINE_AA)
